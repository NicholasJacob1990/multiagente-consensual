---
description: Executa pipeline serial A → B → C com handoffs auditáveis
argument-hint: <etapas, agentes e objetivo>
alias-de: workflow-agentes
profile: pipeline_serial_v1
---
Use a skill `workflow-agentes` com `protocolo = pipeline_serial_v1`. Resolva a sequência, contratos de entrada/saída, modelos, permissões, falha e reentrada. Mostre a prévia e aguarde confirmação. Não avance se uma etapa falhar ou emitir saída inválida.

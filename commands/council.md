---
description: Convoca o Agent Council para deliberar sobre uma questão
argument-hint: "[--with-review] <questão>"
entrypoint: consenso
profile: agent_council
approval-ceiling: consultivo
---

Use `$council` como perfil consultivo da camada `$consenso` para deliberar sobre `$ARGUMENTS`. Preserve opiniões independentes, síntese e registro da sessão. Sem proveniência verificável e `veredito_consenso_v1`, não emita aprovação vinculante.

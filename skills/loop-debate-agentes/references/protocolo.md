...
- quando colegiado: cadeiras e votos válidos, opção vencedora, adesões por proposição essencial, limiar aplicável, `ratio_status`, hashes da opinião principal e das opiniões separadas;
...
Quando `formacao_decisao_colegiada.ativa = true`, exigir também `decisao_colegiada_v1` válido para o mesmo hash: quórum, regra de resultado, votos finais únicos, adesões individualizadas aos fundamentos, política de opiniões separadas, proclamação e gate colegiado. Se `ratio_exigida = true`, somente `ratio_status = unificada` fecha esse gate. `decisao_por_maioria`, `decisao_por_maioria_qualificada` e `somente_resultado` nunca recebem o rótulo `consenso`. A decisão formada é portão intermediário; não promove o arquivo a `canonico_aprovado` sem os demais gates.

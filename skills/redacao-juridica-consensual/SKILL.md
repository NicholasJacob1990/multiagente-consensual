...
O perfil também herda `decisao_colegiada_v1`. Ativá-lo somente para pedido colegiado. Propor `opinion_of_court` como default, com adesão por proposição, `ratio_exigida = true` em decisões destinadas a orientar precedentes e publicação de votos concorrentes e dissidentes. Permitir `seriatim` e `per_curiam` por pedido. Em simulação brasileira, não suprimir voto vencido: publicá-lo no pacote e preservar seu hash. Maioria apenas no dispositivo gera `somente_resultado`, não consenso nem ratio unificada.
